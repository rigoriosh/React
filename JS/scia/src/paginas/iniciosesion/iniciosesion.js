...
class InicioSesion extends React.Component {
  ...
  constructor(props) {
    super(props)
    this.state = { form: { txtCorreo: '', txtClave: '', error: '' }, ubicacion:'', btnSesionActivo: false }
  }
  ...
  handleSubmit = async e => {
    ...
    this.setState({ btnSesionActivo: true })
    ...
    if (respuestaLogin.ok) {
      window.localStorage.setItem('logearUsuario', JSON.stringify({correo: this.state.form.txtCorreo,token:await respuestaLogin.token}))
      this.setState({ form: { txtCorreo: '', txtClave: '', error: '' } })
      window.location.reload()
    } else {
      this.setState({ form: { txtCorreo: this.state.form.txtCorreo, txtClave: '', error: await respuestaLogin.token }, btnSesionActivo: false })
    }
  }
        ...
        <hr/>
          <form onSubmit={this.handleSubmit}>
            ...
            <div className="row">
              ...
              <div className="col-6">
                <RecordarClave ubicacion={this.state.ubicacion}/>
              </div>
            </div>
          </form>
        </div>
