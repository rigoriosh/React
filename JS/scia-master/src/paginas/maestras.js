...
import RecordarClave from './iniciosesion/recordarclave'
...
export default function TablaMaestras(props) {
    ...
    const [rowSeleccionado, setRowSeleccionado] = useState(null)
    const [rowCambiarClave, setRowCambiarClave] = useState(null)
        ...
        const dato = async ()=>{
            ...
            if(apiMaestra!==false&&apiMaestra!==null){
                ...
                apiMaestra.objetosDataColumnName.forEach(info => {
                    if(info.visible)
                        encabezado.push({nombreMostrar: info.nombreMostrar, id: info.nombre, tipoCampo: info.tipoCampo})
                });
                ...
                for (let i = 0; i < encabezado.length; i++) {
                    ...
                    columnas[i] =  { Header: encabezado[i].nombreMostrar, 
                                     accessor: encabezado[i].id.toLowerCase(),
                                     tipoCampo: encabezado[i].tipoCampo}
                }
                        ...
                        apiMaestra.objetosDataColumnName.forEach(info => {
                            ...
                            switch (info.tipoCampo) {
                                ...
                                case 'checklist':
                                    ...
                                    for (let i = 0; i < apiMaestra.objetosDataColumn.length; i++) {
                                        let itemLista = apiMaestra.objetosDataColumn[i]
                                        const subDatos = JSON.parse(objeto[etiqueta])
                                        if(subDatos!==null)
                                            for (let a = 0; a < subDatos.length; a++) {
                                                nombreItem = Object.keys(subDatos[a])
                                                nombreItem = nombreItem[0]
                                                if(subDatos[a][nombreItem]===itemLista.id)
                                                    defaultValue.push(itemLista.descripcion)
                                            }
                                    }
                                    value[etiqueta.toLowerCase()] = {tipo: 'checkListShow', valor:defaultValue, editable: false, nombreItem: nombreItem}
                                    break;
                                default:
                ...
                if(regis!=='id'&&regis!=='acciones'){
                    ...
                    if(typeof campo === 'object' && campo !== null){
                        ...
                        campo.tipo==='email'||campo.tipo==='date'){
                            ...
                            let newObj = []
                            campo.idsOpcion.map(valores=>{
                                if(valores.estado===true){
                                    newObj.push({[campo.nombreItem]:valores.id})
                                }
                            })
                            registro[regis] = JSON.stringify(newObj)
                        }
                    }else{
        ...
        Object.keys(registroPreModificacion).map(key=>{
            if(key!=='acciones'){
                ...
                switch (celda.tipo) {
                    ...
                    case 'checklist':
                        let newObj = []
                        celda.idsOpcion.map(valores=>{
                            if(valores.estado===true){
                                newObj.push({[celda.nombreItem]:valores.id})
                            }
                        })
                        union[key] = JSON.stringify(newObj)
                        break;
                    default:
                        union[key] = celda
                        break;
                ...
                switch (campo.tipoCampo) {
                    ...
                    case 'checklist':
                        let opciones = []
                            let idsOpcion = []
                            for (let i = 0; i < apiMaestra.objetosDataColumn.length; i++) {
                                let datosItem = apiMaestra.objetosDataColumn[i]
                                if(nombreCampo===datosItem.nombre.toLowerCase()){
                                  opciones.push(datosItem.descripcion)
                                  let fueTrue = false
                                  for(let content of valorCelda.valor){
                                    if(content===datosItem.descripcion){
                                        fueTrue = true
                                        idsOpcion.push({estado:true,id:datosItem.id})
                                    }
                                  }
                                  if(!fueTrue){
                                    idsOpcion.push({estado:false,id:datosItem.id})
                                  }
                                }
                            }
                            registro[nombreCampo] =  {tipo:campo.tipoCampo, 
                                                      valor:null, 
                                                      editable: true, 
                                                      opciones: opciones, 
                                                      idsOpcion: idsOpcion,
                                                      nombreItem: valorCelda.nombreItem}
                        break;
                    default:
                        break;
                }
    ...
    const onAgregarRegistro = () => {
        if(nuevoRegistro===null){
            ...
            setTimeout(() => {
                const numPage = (Math.round(data.length/10))-1
                gotoPage(numPage)
    ...
    const onCambiarFiltro = () =>{
        ...
        setData(old=>old.filter((row, index) => { 
            ...
            for (let i = 0; i < filtros.length; i++) {
                 const campo = row[filtros[i].campo]
                //  if(typeof campo === 'object' && campo.tipo=== 'bool'){
                //     if(campo.valor==true && filtros[i].valor.toLowerCase() ==='s'){
                //         contTrue++
                //     }else if(campo.valor==false && filtros[i].valor.toLowerCase() ==='n'){
                //         contTrue++
                //     }
                //  }else 
                if(typeof campo !== 'object' &&campo.indexOf(filtros[i].valor)!==-1){
                    contTrue++
                 }
            }
    ...
    const onCambiarClave = () => {
        document.getElementById('idLinkCambiarClave').click()
    }
    ...
    const onClicTable = (row,index) => {
        // if(rowSeleccionado!==null){
        //     document.getElementById(rowSeleccionado).style.background  = 'var(--bs-table-bg)'
        // }
        // if(nuevoRegistro===null&&registroPreModificacion===null){
        //     setRowSeleccionado(index)
        //     setRowCambiarClave(row)
        //     document.getElementById(index).style.background  = '#0096d2'
        // }
    }
        ...
        <div>
            ...
            <div className="panel panel-default">
                ...
                <div className="panel-body border">
                    ...
                    {estadoBotones.CmdCambiarClave.visible && 
                            <button className="btn btn-light botonesSuperior" 
                                    disabled={JSON.parse(window.localStorage.getItem('offline')).modeOffline} 
                                    onClick={() => onCambiarClave()}>
                                        <span>
                                            <FaLock className="tamanoImg" />
                                            <p className="tamanoLetraImg">Cambiar Clave</p>
                                        </span>
                            </button>}
            ...
            <div style={{overflowX: 'scroll'}}>
                ...
                <Table id="tablaPrincipal" {...getTableProps()} striped bordered hover  style={{ border: 'solid 1px black' }} >
                    <thead>
                        {headerGroups.map(headerGroup => (
                            <tr {...headerGroup.getHeaderGroupProps()}>
                            {headerGroup.headers.map(column => (
                                <th className="text-center m-0 p-1 b-0" {...column.getHeaderProps()}>
                                    <input id={column.render('Header')} 
                                           style={{width: '100%', height: '20px'}} 
                                           onChange={onCambiarFiltro}
                                           hidden={column.render('Header') ==='Acciones' ||
                                                   column.render('tipoCampo') ==='bool'}/>
                                </th>
                            ))}
                            </tr>
                        ))} 
                    ...
                    <tbody {...getTableBodyProps()}>
                        {page.map((row, i) => {
                            prepareRow(row)
                            return (
                            <tr id={i} {...row.getRowProps()} onClick={()=>onClicTable(row,i)}>
                                {row.cells.map(cell => {
                ...
                <Table id="table-to-xls" {...getTableProps()} hidden>
                    <thead>
                        {headerGroups.map(headerGroup => (
                            <tr {...headerGroup.getHeaderGroupProps()}>
                            {headerGroup.headers.map(column => (
                    ...
                    <tbody>
                        {
                            data.map((registro, id)=>{
                                return <tr>
                                    {columns.map(col=>{
                                        return <td>
                                            {(
                                                (data[id][col.accessor]!==null)
                                                ?typeof data[id][col.accessor]=== 'object'
                                                    ?(data[id][col.accessor].tipo==='boton'
                                                        ?''
                                                        :data[id][col.accessor].valor
                                                            ?'Si'
                                                            :'No')
                                                    :data[id][col.accessor]
                                                :'')}
                                        </td>
                                        })
                                    }    
            ...
            <RecordarClave id='idLinkCambiarClave' hidden={true} ubicacion={JSON.parse(window.localStorage.getItem('logearUsuario')).ubicacion} />
