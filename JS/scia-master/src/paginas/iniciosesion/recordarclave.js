...
const RecordarClave = (props) => {
    ...
    const onRecuperar = (e) => { 
        if (/^(([^<>()[\]\.,;:\s@\"]+(\.[^<>()[\]\.,;:\s@\"]+)*)|(\".+\"))@(([^<>()[\]\.,;:\s@\"]+\.)+[^<>()[\]\.,;:\s@\"]{2,})$/i.test(state.txtCorreoRecuperar)){
            if(state.txtCorreoRecuperar!=="" && Array.isArray(e)){
                solicitarClave(state.txtCorreoRecuperar, IPPublicaDispositivo.current)
            } else if(state.txtCorreoRecuperar!=="" && e.target.name === 'idTengoPIN'){
                setState({ ...state, txtPIN: '', txtNuevaClave: '', txtError: '' })
                setShow({ recuperar: false, validar: true, cambiarcontrasena: false })
            }
        } else {
            setState({ ...state, txtError: '¡Por favor ingrese una dirección de correo electrónico!' })
            setShow({ recuperar: true, validar: false, cambiarcontrasena: false })
        }
    ...
    return (
        <div>
            <div  id={props.id} hidden={props.hidden} className="btn btn-link" onClick={() => onRecordarClave()} style={{ float: 'right', color: 'black' }}> Recordar clave <FaKey /></div>
            <Popup state={state}
                show={show.recuperar}
