...
const EditableCell = ({ value: initialValue, row: { index }, column: { id }, updateMyData}) => {
    ...
    const [valueListCheck, setValueListCheck] = useState([])
    ...
    if(typeof initialValue  === 'object' && initialValue!==null){
        ...
        if(initialValue.tipo === 'checklist' && valueListCheck.length===0){
             setValueListCheck(old=>initialValue.idsOpcion)
        }
    }
    ...
    const onCheckList = (id) =>{
        let valor = valueListCheck[id]
        value.idsOpcion[id] = (valor.estado)?{...valor,estado:false}:{...valor,estado:true}
        setValueListCheck((data)=>data.map((row,index)=>{
             return (index === id)?((valor.estado)?{...valor,estado:false}:{...valor,estado:true}):row
         }))
    }
    ...
    if(value!==null && typeof value === 'object'){
        ...
        }else if(value.tipo==='checklist'){
            return <div>
                    {valueListCheck.length!==0 && value.opciones.map(function(dato,i){
                     return <div className="form-check">
                                <input className="form-check-input" type="checkbox" checked={valueListCheck[i].estado} id={dato} onChange={()=>onCheckList(i)}/>
                                <label className="form-check-label" for={dato}>
                                    {dato}
                                </label>
                            </div>
                    })}
                   </div>
        }else if(value.tipo==='checkListShow'){
            return Object.values(value.valor).map(dato=>{
                return <div>{dato}<br/></div>
            })
